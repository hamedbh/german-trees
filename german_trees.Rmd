...
The simple decision tree performs quite well, achieving AUC of `r round(as.numeric(rpart_roc$auc), 3)`.
...
tibble(model = c("Decision Tree", 
	...
	ggplot(aes(x = factor(model), y = AUC, fill = model)) + 
...
Gradient-boosted trees via the XGBoost package perform substantially better than the other models.
