...
Follow-on from the German Trees notebook, which worked through building classification models using the German Credit Data from the [UCI Machine Learning Repository][1] with decision trees, random forests and [XGBoost][2] for modelling, as well as Bayesian Optimisation of hyperparameters. This notebook spends less time on the early steps, misses out random forests entirely, and includes regularised logistic regression methods via `glmnet` and GAMs via `mgcv` to compare results from interpretable methods.

# Data Loading and Setup

```{r load libraries}
# load libraries for the work, comments indicate what each is for
# set a switch that can suppress the startup messages if preferred
show_package_warnings <- TRUE
pkg_list <- c("ggcorrplot", 
              "xgboost", 
              "Matrix", 
              "viridis", 
              "stringi", 
...
`readr::read_delim()` does not convert strings to factors automatically, so we use `forcats` to set factor levels manually: allows for greater control over the levels and means I can give them more informative names from the start. 
...
There was quite a bit of EDA in the German Trees notebook, which we can skip for the most part here. In short:

- `foreign_worker` is nearly homogeneous, should exclude for non-tree models; 
- Some small classes in `purpose`, need to collapse the small ones;
- `sex_status` is no better than the `gender` category, so just keep the latter.

```{r}
clean_df_02 <- clean_df %>% 
    mutate(purpose_simple = fct_lump(purpose, 
                                     prop = 0.1)) %>% 
    select(-sex_status, -purpose)
# which purpose levels have been lumped together into Other?
setdiff(levels(clean_df$purpose), 
        levels(clean_df_02$purpose_simple))
```
...
in_train <- createDataPartition(clean_df_02[["outcome"]], 
                                p = 0.8, 
                                list = FALSE)
train_df <- clean_df_02[in_train, ]
test_df <- clean_df_02[-in_train, ]
```
...
A correlation value of `r round(cor(train_df$amount, train_df$duration), 2)` is a little high, but not so much that we must pick one over the other. For the tree methods this should be even less of a problem. It's something to pay attention to with LASSO, GAM etc.
...
if (file.exists("./bfmodels/rpart_model.rds")) {
	rpart_model <- read_rds("./bfmodels/rpart_model.rds")
} else {
	...
	write_rds(rpart_model, "./bfmodels/rpart_model.rds")
}
...
The simple decision tree performs OK but not brilliantly. We can check the variable importance plot also.
...
Now to XGBoost. There are many hyperparameters to tune, and using grid search through these would not be possible in a reasonable time. Instead I will use Bayesian Optimisation via the `rBayesianOptimization` package.
...
if (file.exists("./bfmodels/xgb_opt_res.rds")) {
    xgb_opt_res <- read_rds("./bfmodels/xgb_opt_res.rds")
} else {
    xgb_opt_res <- BayesianOptimization(xgb_cv_bayes,
                                        ...
                                        eps = 0.0,
                                        verbose = TRUE)
    write_rds(xgb_opt_res, "./bfmodels/xgb_opt_res.rds")
}
xgb_opt_res$Best_Par
```

Now run the cross-validation again with the best parameters found, `eta` reduced by a factor of 100 and `nrounds` increased by the same.

```{r}
xgb_params <- xgb_opt_res[["Best_Par"]]
xgb_param_list <- xgb_params %>% 
    as.list() %>% 
    c(list(objective = "binary:logistic", 
           eval_metric = "auc"))
if (file.exists("./bfmodels/xgb_cv.rds")) {
    xgb_cv <- read_rds("./bfmodels/xgb_cv.rds")
} else {
    ...
    write_rds(xgb_cv, "./bfmodels/xgb_cv.rds")
}
```
...
xgb_factor_preds <- as.character(xgb_preds > 0.5) %>% 
    fct_recode(good = "TRUE", 
               bad = "FALSE")
