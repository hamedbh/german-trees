...
tibble(test_labels, xgb_preds) %>% 
	ggplot(aes(d = test_labels, m = xgb_preds)) + 
	geom_roc() + 
...
Then iterate over the possible thresholds, and get the one that maximises AUC.

```{r}
thresholds <- seq(0, 1, 0.001)
auc_scores <- map_dbl(thresholds, 
                      function(x) {
                          tmp_preds <- as.integer(xgb_preds > x)
